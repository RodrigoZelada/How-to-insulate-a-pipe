...
i = format(0,'04d')
meshes='meshes'
M0 = Mesh3D(meshes+"/MarcThCylinderCodo.mesh")
...
phi12 = lambda x,y,z : max( (np.cos(theta)**2)*(x-xcen2)**2 + (y-0.5)**2 + (np.sin(theta)**2)*(z-zmin)**2 - 2*np.cos(theta)*np.sin(theta)*(x-xcen2)*(z-zmin) - r**2,z-0.78,0.22-z )
...
phi2 = lambda x,y,z: max((z-0.5)**2 + (y-0.5)**2 - 0.45**2,cut(x,y,z)) #(z-0.5)**2 + (y-0.5)**2 - 0.45**2 
...
Thinit = mmg3d(M1,hmin,hmax,hgrad,hausd,ls=True,sol=phiinit2,params=refermmg,extra_args="-rmc 1e-6",debug=1)
...
varf stokes([ux, uy, uz, p], [wx, wy, wz, qq]) =
   ...
   +on(1,ux=(0.1^2 - (y-0.5)^2 - (z-0.75)^2)/0.01, uy=0., uz=0.);
...
func cut=min(max((y-0.5)^2 + (z-0.75)^2 - 0.2^2, x-0.02), max((y-0.5)^2 + (z-0.25)^2 - 0.2^2,0.98-x) );
