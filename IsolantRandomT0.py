...
barrier = lambda x,y,z: min(max((y-0.5)**2 + (z-0.5)**2 - R**2, x-eps), max((y-0.5)**2 + (z-0.5)**2 - R**2,1-eps-x) ) #(z-0.5)**2 + (y-0.5)**2 - 0.105**2
...
set(AG, sparams = "-ksp_max_it 99 -ksp_rtol 1e-10 -pc_type hypre"); //"-pc_type lu"
...
def computeU(mesh):
    ...
    os.system(f"time mpiexec -n 1 ./IsolantRandomT0 {output} -pc_type lu -pc_factor_mat_solver_type superlu_dist -ksp_monitor ")
...
PhBox dOmegaBox;
Ph2 T2,R2,T21,T22,R21,R22,T0,T01,T02;

T2[] = readsol("$OUTPUT/T2.sol");
R2[] = readsol("$OUTPUT/R2.sol");
T0 = 0.;

T21[] = readsol("$OUTPUT/T21.sol");
R21[] = readsol("$OUTPUT/R21.sol");
T01 = 20.*x;

T22[] = readsol("$OUTPUT/T22.sol");
R22[] = readsol("$OUTPUT/R22.sol");
T02 = 10.*z;

dOmegaBox[] = readsol("$OUTPUT/d2.sol");
if (mpirank==0){
    savevtk("$OUTPUT/T2.vtu",Th2,T2);
    savevtk("$OUTPUT/T21.vtu",Th2,T21);
    savevtk("$OUTPUT/T22.vtu",Th2,T22);
}
...
set(A, sparams = "-ksp_max_it 100 -ksp_rtol 1e-10 -pc_type hypre"); //"-pc_type lu"
...
class PureThermalHeatConduction(Optimizable):
    ...
    def accept(self,results):
        ...
        M = Mesh3D(mesh)
        if niter >= 1:
            shutil.copyfile(output+'/T2.vtu',output+'/T2_'+format(niter-1,'03d')+'.vtu')
            shutil.copyfile(output+'/T21.vtu',output+'/T21_'+format(niter-1,'03d')+'.vtu')
            shutil.copyfile(output+'/T22.vtu',output+'/T22_'+format(niter-1,'03d')+'.vtu')
