...
i = format(0,'04d')
meshes='meshes'
M0 = Mesh3D(meshes+"/CylinderCodoZOrdered.mesh")
...
xcen1 = 0.75#0.866
xcen2 = 0.25 #0.134
theta = 0.105*np.pi #0.33

barrier3 = lambda x,y,z: min(max((z-zmax)**2 + (y-0.5)**2 - (r + eps)**2, x-xcen1), max( (np.cos(theta)**2)*(x-xcen2)**2 + (y-0.5)**2 + (np.sin(theta)**2)*(z-zmin)**2 - 2*np.cos(theta)*np.sin(theta)*(x-xcen2)*(z-zmin) - (r+eps)**2,z-zmax,zmin-z ), max((z-zmin)**2 + (y-0.5)**2 - (r + eps)**2, xcen2-x) )
barrier4 = lambda x,y,z : (z-zmax)**2 + (y-0.5)**2 +(x-xcen1)**2 - (r+eps)**2
barrier5 = lambda x,y,z : (z-zmin)**2 + (y-0.5)**2 +(x-xcen2)**2 - (r+eps)**2
barrier = lambda x,y,z: min(max((y-0.5)**2 + (z-zmax)**2 - R**2, x-eps), max((y-0.5)**2 + (z-zmin)**2 - R**2,1-eps-x) ) #min(barrier3(x,y,z),barrier4(x,y,z),barrier5(x,y,z))

phi11 = lambda x,y,z : max((z-zmax)**2 + (y-0.5)**2 - r**2, x-xcen1)
phi12 = lambda x,y,z : max( (np.cos(theta)**2)*(x-xcen2)**2 + (y-0.5)**2 + (np.sin(theta)**2)*(z-zmin)**2 - 2*np.cos(theta)*np.sin(theta)*(x-xcen2)*(z-zmin) - r**2,z-1.68,0.32-z, x-0.95,0.05-x )
phi13 = lambda x,y,z : max((z-zmin)**2 + (y-0.5)**2 - r**2, xcen2-x)
phi14 = lambda x,y,z : (z-zmax)**2 + (y-0.5)**2 +(x-xcen1)**2 - r**2
phi15 = lambda x,y,z : (z-zmin)**2 + (y-0.5)**2 +(x-xcen2)**2 - r**2
phi1 = lambda x,y,z: min(phi11(x,y,z), phi12(x,y,z),  phi13(x,y,z), phi14(x,y,z), phi15(x,y,z))
phi21 = lambda x,y,z : max((z-zmax)**2 + (y-0.5)**2 - R**2, x-xcen1) #max(-x+eps,(z-zmax)**2 + (y-0.5)**2 - R**2, x-xcen)
phi22 = lambda x,y,z : max( (np.cos(theta)**2)*(x-xcen2)**2 + (y-0.5)**2 + (np.sin(theta)**2)*(z-zmin)**2 - 2*np.cos(theta)*np.sin(theta)*(x-xcen2)*(z-zmin) - R**2,z-zmax,zmin-z )
phi23 = lambda x,y,z : max((z-zmin)**2 + (y-0.5)**2 - R**2, xcen2-x) #max(x+eps-1.,(z-zmin)**2 + (y-0.5)**2 - R**2, xcen-x)
phi24 = lambda x,y,z : (z-zmax)**2 + (y-0.5)**2 +(x-xcen1)**2 - R**2
phi25 = lambda x,y,z : (z-zmin)**2 + (y-0.5)**2 +(x-xcen2)**2 - R**2

cutIn = lambda x,y,z: min((y-0.5)**2 + (z-zmax)**2 - R**2, eps-x)
cutOut = lambda x,y,z: min((y-0.5)**2 + (z-zmin)**2 - R**2, x-1+eps)
cut = lambda x,y,z: max(cutIn(x,y,z),cutOut(x,y,z))

#phi2 = lambda x,y,z: max((z-1.)**2 + (y-0.5)**2 - 0.9**2,0.2-y,y-0.8) #min(phi21(x,y,z), phi22(x,y,z),  phi23(x,y,z), phi24(x,y,z), phi25(x,y,z)) #
phi2 = lambda x,y,z: max((z-1.)**2 + (y-0.5)**2 - 0.9**2,cut(x,y,z)) 
...
Thinit = mmg3d(M1,hmin,hmax,hgrad,hausd,ls=True,sol=phiinit2,params=refermmg,extra_args="-rmc 1e-7",debug=1)
...
int empty = 2;
real k1=0.15; //0.15*1e-6; //thermal diffusivity D or alpha of water;
real k2=0.1; //0.1*1e-6; //thermal diffusivity D or alpha of rubber;
real vtarget=0.2993584; 
...
varf stokes([ux, uy, uz, p], [wx, wy, wz, qq]) =
   ...
   +on(1,ux=(0.1^2 - (y-0.5)^2 - (z-1.75)^2)/0.01, uy=0., uz=0.);
...
FreeFemRunner([preamble, solve_ns],config,run_dir=output,run_file='solve_ns.edp',debug=1).execute({'MESH':output+'/Th00.mesh'},ncpu=N)
...
def computeU(mesh):
    ...
    os.system(f"time mpiexec -n 1 ./MatrixBinaryPetsc {output} -pc_type lu -pc_factor_mat_solver_type superlu_dist -ksp_monitor ")
...
func cut=min(max((y-0.5)^2 + (z-1.75)^2 - 0.2^2, x-0.02), max((y-0.5)^2 + (z-0.25)^2 - 0.2^2,0.98-x) );
...
/*varf rieszJ([dpxJ,dpyJ,dpzJ],[thetaxp,thetayp,thetazp])= int3d(ThBox,qforder=3,optimize=0)(gammaR^2*GRAD(dpxJ,dpyJ,dpzJ)'*GRAD(thetaxp,thetayp,thetazp)+[dpxJ,dpyJ,dpzJ]'*[thetaxp,thetayp,thetazp])
                                             +int2d(ThBox,10)(gammaR^2*gradtauN(dpxJ,nx2,ny2,nz2)'*gradtauN(thetaxp,nx2,ny2,nz2) + gammaR^2*gradtauN(dpyJ,nx2,ny2,nz2)'*gradtauN(thetayp,nx2,ny2,nz2) + gammaR^2*gradtauN(dpzJ,nx2,ny2,nz2)'*gradtauN(thetazp,nx2,ny2,nz2))
...
varf rieszH([dpxVol,dpyVol,dpzVol],[thetaxp,thetayp,thetazp])=int3d(ThBox,qforder=3,optimize=0)(gammaR^2*GRAD(dpxVol,dpyVol,dpzVol)'*GRAD(thetaxp,thetayp,thetazp)+[dpxVol,dpyVol,dpzVol]'*[thetaxp,thetayp,thetazp])
                                                           +int2d(ThBox,10)(gammaR^2*gradtauN(dpxVol,nx2,ny2,nz2)'*gradtauN(thetaxp,nx2,ny2,nz2) + gammaR^2*gradtauN(dpyVol,nx2,ny2,nz2)'*gradtauN(thetayp,nx2,ny2,nz2) + gammaR^2*gradtauN(dpzVol,nx2,ny2,nz2)'*gradtauN(thetazp,nx2,ny2,nz2))
...
def shapeUpdate(mesh, xi):
    ...
    refermmg = f"""
                ...
                13 Triangles 5e-3 2.5e-2 1e-3
    """
    newM=mmg3d(M,hmin,hmax,hgrad,hausd,ls=True,sol=phiNew,params=refermmg,extra_args="-rmc 1e-7 ",debug=1)
...
optSettings = {'dt':2*hmin,'alphaJ':2.,'alphaC':1., 'maxit':500, 'provide_gradient': True, 'maxtrials':1,
              'itnormalisation': 3}
