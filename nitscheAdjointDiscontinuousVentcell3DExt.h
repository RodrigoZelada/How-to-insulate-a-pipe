void nitscheAdjointDiscontinuousVentcell3D(Mat A, Vec b, mesh3D &Th, vector<double> &G, vector<double> &kappa, vector<vector<double>> &u, map<int,int> Gamma, mesh3D &Th2, DirichletBC &DBC1, DirichletBC &DBC2)
{
	int N = Th.ntet;
	int Nt = Th.nt;
	...
	double kRegion, kOtherRegion, sign;
	int region;
				...
				for (i=0; i<D; i++){
					...
					for (int p=0; p<D; p++){
						...
						for (int q=0; q<p; q++){
							...
							vector<double>  xq1 = {Th.vertices[qq][0], Th.vertices[qq][1], Th.vertices[qq][2],1.};
							vector<double>  xpq1 = {0.5*xp1[0] + 0.5*xq1[0], 0.5*xp1[1] + 0.5*xq1[1], 0.5*xp1[2] + 0.5*xq1[2],1.};
							val += (area/3.)*dot(coefii,xpq1)*(0.5*G[pp] + 0.5*G[qq]);
						}
					}
					if ( iibelongDBC == 0 ){
